...
\newcommand{\studentone}{Student~1 Name, Matriculation~\#, student1@stud.hslu.ch}
\newcommand{\studenttwo}{Student~2 Name, Matriculation~\#, student2@stud.hslu.ch}
\newcommand{\studentthree}{Student~3 Name, Matriculation~\#, student3@stud.hslu.ch}
\newcommand{\examinerone}{First reviewer's title and name}
\newcommand{\examinertwo}{Second reviewer's title and name}
\newcommand{\examinerthree}{Third reviewer's title and name}
...
\begin{titlepage}
  ...
  \begin{center}
  \begin{tabular}{r l}
    \textbf{Group Name:}  & \groupname \\[1em]
...
The global oil market is highly sensitive to geopolitical tensions, and
few geographies have as sustained an impact on price dynamics as the
Middle East and in particular Iran. This project investigates the extent
to which conflict events involving Iran propagate into observable
changes in oil prices, oil-company stock returns, and the volatility of
both. The intended audience is geopolitical risk analysts and commodity
traders who need to understand how escalations in the region
(sanctions, military engagements, Strait of Hormuz incidents, nuclear
negotiations) translate into oil market movements in near real time.
...
The 2024--2025 Iran-Israel escalation cycle has renewed analytical
interest in quantifying how specific classes of geopolitical events
transmit into market prices. This project contributes a reproducible,
cloud-native data pipeline that joins high-frequency stock data,
authoritative oil-price series, and classified news events to enable
structured analysis across event types.
...
\begin{figure}[H]
  ...
  \caption{End-to-end data-lake-and-warehouse architecture. The
    ...
    queryable from Tableau before the warehouse load completes
...
\begin{itemize}
  ...
  \item AWS Lambda for stateless compute (Python~3.11 runtime,
  ...
  \item AWS Step Functions for orchestration with retries.
...
\paragraph{Authentication.}
Requests are authenticated with a Gemini API key stored in AWS Secrets
Manager under the secret \texttt{dwl/news/gemini}. The ingest Lambda
retrieves the key at cold-start time via \texttt{secretsmanager:GetSecretValue}
and reuses the cached value for all weekly calls within the same
invocation, eliminating repeated Secrets Manager round-trips.

\paragraph{Rate limits.}
The Gemini 2.5 Flash free tier permits fifteen requests per minute.
The ingest Lambda processes one weekly chunk per request and inserts a
five-second sleep between successive calls to stay within this
threshold. A single retry is attempted on an HTTP~429 response after a
thirty-second back-off; if the retry also fails the week is recorded as
an empty partition and an SNS alert is published for manual follow-up.

\paragraph{Pagination.}
Not applicable. Each API call covers a bounded seven-day window and
returns at most fifteen events in a single response, well within the
model's output token limit.
...
ultimately trigger an SNS alert for human follow-up.
...
AWS Lambda (Python 3.12), EventBridge, Step Functions, S3, Secrets
...
Step Functions Retry blocks per state handle transient errors.
Non-retryable errors are caught and routed to SNS for follow-up.
...
reads for the date-range queries that drive the Q1 to Q3 dashboards.
...
The \texttt{news\_events} schema contains one fact table and two small
dimensions. The fact table \texttt{news\_events.fact\_conflict\_daily}
has a grain of one row per calendar day. Its primary idempotency key is
\texttt{event\_date}; \texttt{date\_sk} is a foreign key to the
conformed \texttt{analytics.dim\_date} dimension shared with the stocks
and oil domains. Analytical columns record the daily event count,
maximum and average severity, a conflict presence flag, the top event
summary and category, per-category event counts and maximum severities
across all five taxonomy categories, a \texttt{hormuz\_flag} for
Strait-of-Hormuz-related events, and an \texttt{ingest\_timestamp}.
Two surrogate foreign keys, \texttt{category\_sk} and
\texttt{severity\_sk}, link to the dimension tables described below and
support star-schema joins in the cross-domain marts.

\texttt{dim\_event\_category} has five static rows mapping the
taxonomy codes (\texttt{military\_conflict}, \texttt{sanctions},
\texttt{security\_incidents}, \texttt{nuclear\_diplomacy},
\texttt{diplomatic\_shifts}) to integer surrogate keys 1--5.
The key assignment matches the \texttt{CATEGORY\_SK\_MAP} baked into
the transform Lambda, so gold Parquet \texttt{category\_sk} values
resolve correctly against the dimension without any runtime lookup.
\texttt{dim\_severity} has five rows mapping the 1--5 severity scale
to business labels from Routine through Extreme, providing a
human-readable join target for Tableau filters and tooltip labels.

\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{figures/news_er.PNG}
  \caption{Entity-relationship diagram for the \texttt{news\_events} star
    schema, introspected from the live \texttt{dwl-shared-pg} RDS
    instance. The central fact \texttt{news\_events.fact\_conflict\_daily}
    carries one row per calendar day and three foreign keys:
    \texttt{date\_sk} to the conformed \texttt{analytics.dim\_date}
    dimension, \texttt{category\_sk} to the five-row
    \texttt{dim\_event\_category} dimension, and \texttt{severity\_sk}
    to the five-row \texttt{dim\_severity} dimension.}
  \label{fig:news_er}
\end{figure}
...
The conflict-sessions mart shares the same calendar-day-by-ticker grain
but enriches each row with session context. A session is defined by
severity-5 events: any two severity-5 days that are no more than seven
calendar days apart are merged into the same session. Each session
window then expands three days before the first severity-5 event and
three days after the last, capturing the pre-escalation and
post-escalation market movement. Each row carries
\texttt{session\_id}, \texttt{session\_start}, \texttt{session\_end},
\texttt{first\_sev5\_date}, \texttt{last\_sev5\_date}, and
\texttt{session\_length\_days}. Brent and WTI prices are
forward-filled across non-trading days so every row has an oil price
regardless of the market calendar. This mart supports the Q1 and Q2
dashboards by making extreme-conflict episodes directly filterable and
comparable in Tableau without requiring session logic inside the
workbook.

The category-impact mart is aggregated one level further. Its grain is
one row per stock and top event category, where the category is taken
...
post-event return, average absolute move, and maximum drawdown or
equivalent downside measure. This design keeps the expensive
aggregation calculations out of Tableau and gives the workbook compact
tables optimised for filtering and ranking.
...
\begin{figure}[H]
  ...
  \caption{Analytics marts implemented as cross-domain SQL views over warehouse facts. Each mart joins all four source tables. \texttt{mart\_daily\_signals} provides the raw joined time series; \texttt{mart\_conflict\_sessions} groups severity-5 events within 7-day gaps into sessions with a $\pm$3-day window and forward-filled oil prices; \texttt{mart\_category\_impact} pre-aggregates return statistics by conflict category.}
...
Oil-price behaviour during conflict periods (Q1, BI1), individual
stock reactivity and divergence across the stock universe (Q2, BI2),
and identification of the most impactful specific events (Q3, BI3).
...
The Tableau workbook is built as a single integrated dashboard with
four linked panels. A Brent price timeline spanning the full analysis
window occupies the top of the dashboard; conflict sessions are
highlighted in red, making escalation periods immediately visible
against the price trend. A drag-to-select action on the timeline
simultaneously updates the three detail panels: a top-events table
showing event summaries and severity scores for the selected period;
a Brent and WTI price chart for the selected scope; and a bar chart
of log stock returns for each of the fifteen stocks, coloured green
for positive and red for negative returns. The multi-select filter
allows the reader to isolate specific stocks for direct comparison.

\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{figures/dashboard_overview.PNG}
  \caption{Full dashboard at rest showing the complete Brent price
...
\begin{figure}[H]
  ...
  \caption{Dashboard zoomed into a conflict session. All four panels
...
\begin{figure}[H]
  ...
  \caption{Top-events panel during a conflict session, showing event
...
\begin{figure}[H]
  ...
  \caption{Stock return bar chart during a conflict session. Green
    bars indicate positive log returns, red bars negative. The
    divergence across stocks is the dashboard's central analytical
    finding.}
...
\subsubsection{Q1: Oil price behaviour during conflict periods (S2 + S3)}
\label{sec:answer_q1}
...
Comparing the Brent and WTI series in the bottom-left panel reveals
that the two benchmarks move closely in tandem during conflict
periods, with no persistent divergence between them. This is
consistent with their shared exposure to Middle East supply-risk
sentiment, despite the two grades having different delivery locations
and demand bases.

\subsubsection{Q2: Individual stock divergence during conflict periods (S2 + S3)}
\label{sec:answer_q2}

The most prominent finding of the dashboard is the degree to which
individual stocks diverge from one another during conflict sessions.
As shown in Figure~\ref{fig:dash_stocks}, the fifteen stocks produce
a wide spread of positive and negative returns within the same session
window, rather than moving as a uniform block. This divergence is not
random: stocks with direct exposure to shipping and freight benefit
from the supply-risk premium that conflict introduces, while other
segments of the energy complex react negatively or remain flat. The
result challenges a simple ``oil conflict lifts all energy stocks''
assumption and points to meaningful variation in how different
business models within the sector translate geopolitical risk into
market returns.

\subsubsection{Q3: Most impactful individual conflict events (S3)}
\label{sec:answer_q3}

The top-events panel (Figure~\ref{fig:dash_events}) identifies which
specific classified events occurred within each selected session
window, together with their severity score. Navigating across sessions
in the dashboard reveals that the largest concurrent oil-price and
stock-return movements cluster around events in the
\texttt{military\_conflict} and \texttt{security\_incidents}
categories. Diplomatic and sanctions events of lower severity are
visible in the events panel during quieter periods but do not produce
comparably large price responses in the oil or equity panels.
...
state from analytics-ready state. The tiered 15-stock universe is
the key analytical choice making the Q2 stock-divergence finding
possible.
...
limits the number of observable conflict sessions. The HSLU sandbox account also
