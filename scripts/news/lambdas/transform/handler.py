...
Silver : s3://dwl-datapowerchords-curated/news/events_parquet/
         ...
         One Parquet file per month, one row per event, no filtering.

Gold   : s3://dwl-datapowerchords-gold/news/fact_conflict_daily/
             event_date=<YYYY-MM-DD>/data.parquet
         One Parquet file per day, one row per day, aggregated.
         Filters: drop date_confidence=approximate, drop severity=1.
...
def handler(event: dict, context) -> dict:
    """
    Routine run  : event = {}  — processes current month silver + past 7 days gold
